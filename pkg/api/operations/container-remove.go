...
func ContainerRemove(controllerMailbox actor.MailboxSender[controller.Message]) usecase.Interactor {
	type request struct {
	...
	u := usecase.NewInteractor(
		func(ctx context.Context, request request, response *response) error {
			...
			reply := make(chan messages.ContainerRemoveResponse)
			controllerMailbox.Send(
				ctx,
				&messages.ContainerRemoveRequest{
					Context:     ctx,
					ReplyTo:     reply,
	...
	u.SetName("container_remove")
	u.SetTitle("Remove a container")
	u.SetTags("container")
