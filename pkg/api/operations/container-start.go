...
func ContainerStart(controllerMailbox actor.MailboxSender[controller.Message]) usecase.Interactor {
	type request struct {
	...
	u := usecase.NewInteractor(
		func(ctx context.Context, request request, response *response) error {
			...
			reply := make(chan messages.ContainerStartResponse)
			controllerMailbox.Send(
				ctx,
				&messages.ContainerStartRequest{
					Context:     ctx,
					ReplyTo:     reply,
	...
	u.SetName("container_start")
	u.SetTitle("Start a container")
	u.SetTags("container")
