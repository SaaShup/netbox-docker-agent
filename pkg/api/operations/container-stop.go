...
func ContainerStop(controllerMailbox actor.MailboxSender[controller.Message]) usecase.Interactor {
	type request struct {
	...
	u := usecase.NewInteractor(
		func(ctx context.Context, request request, response *response) error {
			...
			reply := make(chan messages.ContainerStopResponse)
			controllerMailbox.Send(
				ctx,
				&messages.ContainerStopRequest{
					Context:     ctx,
					ReplyTo:     reply,
	...
	u.SetName("container_stop")
	u.SetTitle("Stop a container")
	u.SetTags("container")
